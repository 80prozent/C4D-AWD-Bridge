...
def buildGeometryStreams(subMesh,scale):
    ...
    if len(subMesh.vertexBuffer)>0:
        ...
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(1,pointData))
    if len(subMesh.normalBuffer)>0:
        ...
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(4,normalData))
    if len(subMesh.quadBuffer)>0:
        quadData=[]
        for indexPoint in subMesh.quadBuffer:
            quadData.append(indexPoint)
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(8,quadData))
    if len(subMesh.indexBuffer)>0:
        indexData=[]
        for indexPoint in subMesh.indexBuffer:
            indexData.append(indexPoint)
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(2,indexData))
    if len(subMesh.uvBuffer)>0:
        uvData=[]
        for uv in subMesh.uvBuffer:
            uvData.append(uv.x)
            uvData.append(uv.y)
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(3,uvData))
    if len(subMesh.weightsBuffer)>0 and len(subMesh.jointidxBuffer)>0:
        ...
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(6,indexData))
        ...
        subMesh.saveGeometryStreams.append(awdBlocks.awdGeometryStream(7,weightData))
