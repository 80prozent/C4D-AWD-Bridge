...
class mainScene(object):
    ...
    def createMaterialBlock(self,materialID,colormat):
        if int(materialID)>=0:
            ...
            newAWDBlock=classesAWDBlocks.StandartMaterialBlock(self.idCounter,0,colormat)
    ...
    def createSingleTextureBlock(self,texturePath):
        ...
        if isInList==None:
            ...
            newAWDBlock=classesAWDBlocks.TextureBlock(self.idCounter,0,str(texturePath),self.embedTextures,pathName)
            self.IDsToAWDBlocksDic[str(self.idCounter)]=newAWDBlock
            self.allAWDBlocks.append(newAWDBlock)
            ...
            if curObj.GetType()==c4d.Oinstance:
                ...
                for selectionTag in curObj[c4d.INSTANCEOBJECT_LINK].GetTags():
                    if selectionTag.GetType()==c4d.Tpolygonselection:
                        allSelections.append(mainHelpers.PolySelection(selectionTag.GetName(),selectionTag.GetBaseSelect().GetAll(len(curObj[c4d.INSTANCEOBJECT_LINK].GetAllPolygons())))) 
                
                if hasTextures==True:
                    materials=mainHelpers.getObjectsMaterials(curObj[c4d.INSTANCEOBJECT_LINK],None,None,self)
                if hasTextures==False:
                    materials=mainHelpers.getObjectsMaterials(curObj,allSelections,None,self)
                
            if curObj.GetType()==c4d.Opolygon or fromChild==True:
                materials=mainHelpers.getObjectsMaterials(curObj,None,None,self)
    ...
    def createAllSceneBlocks(self,objList,parentExportSettings=None,tagForExport=True,returner=True):
        for object in objList:
            ...
            if exporterSettingsTag==None:
                returnerAR=mainParseObjectToAWDBlock.createSceneBlock(self,object,tagForExport,returner,False)
