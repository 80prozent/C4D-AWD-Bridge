...
def saveLightXML(exportData):
    ...
    while lightPickerCnt<len(exportData.lightPickerList):        
        ...
        nameCnt=0
        for name in exportData.lightPickerList[lightPickerCnt].lights:
...
def getObjectLights(curObj,exportData):
    ...
    if len(exportData.lightPickerDic)==0:
        exportData.lightPickerDic[str("")]=LightRouter()
        exportData.lightPickerList.append(LightRouter())
    if lightPickerString!="":
        ...
        if getLightPicker is None:
            newLightPicker=LightRouter()
            newLightPicker.name="LightPicker "+str(len(exportData.lightPickerList))
            newLightPicker.lights=newLightsList
            exportData.lightPickerDic[str(lightPickerString)]=len(exportData.lightPickerList)
            returnIdx=len(exportData.lightPickerList)
            exportData.lightPickerList.append(newLightPicker)
