    ...
    for faceIndex in xrange(count):                                     # iterate trough all polygons 
        ...
        while subcount < allSubMeshCount:# loop trough all SubMeshes
            if meshBlock.saveSubMeshes[subcount].selectionIndexe[faceIndex]==1:  # if the polygon is selected by this submesh, the polygon will be insterted into this submesh
                subcount=checkForRessourceLimits(exportData,meshBlock,subcount,faceIndex,faceStyle,meshBlock.saveSubMeshes[subcount]) # check if this submesh has allready reached its limits. if it allready has, the index of the submesh to use instead is returned
                if usePhong==True:    # if phongshading should be considered, execute the "buildSharedPoint()" for each of the points of the polygon
                    buildSharedPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,allOldPoints[oldpoints.a],uva,meshBlock.saveSubMeshes[subcount],oldpoints.a,normala,normalf,phoneAngle,usePhongAngle,isEdgeBreakA)
                    buildSharedPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,allOldPoints[oldpoints.b],uvb,meshBlock.saveSubMeshes[subcount],oldpoints.b,normalb,normalf,phoneAngle,usePhongAngle,isEdgeBreakB)
                    buildSharedPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,allOldPoints[oldpoints.c],uvc,meshBlock.saveSubMeshes[subcount],oldpoints.c,normalc,normalf,phoneAngle,usePhongAngle,isEdgeBreakC)
                    if str(faceStyle)=="quad":
                        buildSharedPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,allOldPoints[oldpoints.d],uvd,meshBlock.saveSubMeshes[subcount],oldpoints.d,normald,normalf,phoneAngle,usePhongAngle,isEdgeBreakD)
                
                if usePhong==False: # if phongshading should not be considered, execute the "buildPoint()" for each of the points of the polygon
                    buildPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,meshBlock.saveSubMeshes[subcount],allOldPoints[oldpoints.a],oldpoints.a,uva)
                    buildPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,meshBlock.saveSubMeshes[subcount],allOldPoints[oldpoints.b],oldpoints.b,uvb)
                    buildPoint(exportData,faceStyle,meshBlock,meshBlock.sceneObject,meshBlock.saveSubMeshes[subcount],allOldPoints[oldpoints.c],oldpoints.c,uvc)
                    if str(faceStyle)=="quad":
...
def checkForRessourceLimits(exportData,meshBlock,subcount,faceIndex,faceStyle,curSubMesh):
